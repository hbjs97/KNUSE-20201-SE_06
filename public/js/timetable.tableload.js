...
_gfn = _.extend(_gfn, {
	...
	insertSubjectIntoTable: function (subject, callback) {
		...
		// 시간 없는 수업 체크
		subject.times = _.reject(subject.times, function (time) {
			return time.endtime === 0;
		});
		if (!subject.times.length) {
			alert('시간이 정해지지 않은 수업은 추가할 수 없습니다.');
			return false;
		}
	...
	deleteSubjectInTable: function (id, callback) {
		...
		var $tbody = $container.find('table.tablebody');
		var $colsDivs = $tbody.find('div.cols');
		var $subjectDivs = $colsDivs.find('div.subject').filter(function () {
	...
	var _fn = {
		initiate: function () {
			...
			if ($('body').is(':has(#subjects)') || $('body').is(':has(#customsubjects)')) {
				$container.on('mouseenter', 'table.tablebody div.subject', function () {
					$(this).find('ul.status').show();
				}).on('mouseleave', 'table.tablebody div.subject', function () {
					$(this).find('ul.status').hide();
				});
				$container.on('click', 'table.tablebody div.subject ul.status li.del', function () {
