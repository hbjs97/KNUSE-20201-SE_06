      ...
      $(data).find('response > subject').each(function () {
        ...
        _.each(_set.subjectColumnInfo, function (item) {
          ...
          if (item.name === 'lectureRate') {
            ...
            var lectureUrl = 'javascript: alert("아직 등록된 강의평이 없습니다.");';
            if (lectureId !== '0') {
              lectureUrl = '/lecture/view/' + $subjectData.attr('lectureId');
            }
            var percent = Number(value) / 5 * 100 + '%';
            var $star = $('<a></a>').addClass('star').attr({href: lectureUrl, target: '_blank', title: value}).appendTo($td);
