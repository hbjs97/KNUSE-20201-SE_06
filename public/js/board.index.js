...
$().ready(function () {
	...
	var _fn = {
		...
		loadContent: function (params) {
			if (params.v) {
				$container.find('div.seasons, div.categories, div.topics').addClass('none');
				...
				$container.find('div.topics')[_set.searchType > 0 ? 'addClass' : 'removeClass']('none');
				_fn.loadArticles();
			}
		},
		...
		createMoimInfo: function (data) {
			...
			$('<a></a>').attr('href', '/' + _set.boardId).html(boardName).appendTo($h1);
			...
			$('<a></a>').attr('href', '/' + _set.boardId).html(boardName).appendTo($containerH1);
			if (boardInfo) {
		...
		createTopics: function ($topicsXml) {

			var $topics = $('<div></div>')
			.addClass('wrap topics')
			.insertAfter($containerTitle);

			$topicsXml.find('topic').each(function () {
				var data = {
					id: $(this).attr('id'),
					name: $(this).attr('name'),
					is_active: Number($(this).attr('is_active')),
					is_new: Number($(this).attr('is_new'))
				};
				_set.topics.push(data);
				var $topic = $('<div></div>')
				.addClass('topic')
				.data(data)
				.html('<span>' + data.name + '</span>')
			...
			$topics[_set.searchType > 0 ? 'addClass' : 'removeClass']('none');
		...
		loadArticles: function () {
			...
			_fn.ajaxArticles(function (data) {
				if (_set.moiminfo) {
					...
					if ($responseXml.is(':has(topics)')) {
						_fn.createTopics($responseXml.find('topics'));
					}
					_set.moiminfo = false;
				...
				$container.find('div.topics div.topic').each(function (idx, elem) {
					$(elem)[Number($(elem).data('id')) === _set.topicId ? 'addClass' : 'removeClass']('selected');
				});
			});
		},
		ajaxArticles: function (callback) {
			...
			if (_set.topicId > 0) {
				conditions.topic_id = _set.topicId;
			}
			$.ajax({
				url: _apiServerUrl + '/find/board/article/list',
		...
		createArticles: function (data, isListItem) {
			...
			$(data).find('article').each(function () {
				...
				if ($this.attr('board_id')) {
					...
					$boardname = $('<a></a>').attr('href', boardUrl).html('from ' + $this.attr('board_name')).addClass('boardname');
				}
				if (isListItem) {
					var articleUrl;
					...
					if ($attachesData.length > 0) {
						...
						$attachesData.each(function () {
							...
							$('<img>').attr('src', $attachData.attr('fileurl')).on('click', function () {
								window.open($attachData.attr('imageUrl'), '_blank');
							}).appendTo($figure);
			...
			if (_set.boardPage === 1 && _set.isSearchable && !(_set.categoryId > 0 || _set.topicId > 0)) {
		...
		setTopic: function (that) {
			var topicId = Number($(that).data('id'));
			var boardUrl = _fn.encodeUrl({ boardId: _set.boardId });
			if (location.pathname !== boardUrl) {
				if (typeof history.pushState !== 'undefined') {
					_set.boardPage = 1;
			...
			_set.searchType = 0;
			_set.keyword = '';
			_set.topicId = topicId === _set.topicId ? 0 : topicId;
			_fn.loadArticles();
		},
		...
		showWriteArticleForm: function ($article) {
			...
			if (!$article) {
				...
				if (_set.topics.length > 0) {
					var $topicsContainer = $('<p></p>')
					.addClass('topics inactive')
					.append(
						$('<div></div>').addClass('direction').text('혹시 이 주제에 대한 글인가요?')
					)
					.insertAfter($text.parent());
					_.each(_set.topics, function (topic) {
						var $topicRadio = $('<input>').attr({
							type: 'radio',
							name: 'topic_id',
							id: 'topic_' + topic.id,
							value: topic.id
						}).addClass(topic.is_active > 0 ? 'active' : 'inactive');
						var $topicLabel = $('<label></label>').attr({
							for: 'topic_' + topic.id
						}).text(topic.name);
					...
					$('<label></label>').addClass('more').text('더보기').appendTo($topicsContainer).on('click', function () {
						$(this).remove();
						$topicsContainer.removeClass('inactive');
					});
					var topic_id = '0';
					$($form[0].topic_id).on('click', function (event) {
						if (topic_id === $form[0].topic_id.value) {
							$(event.currentTarget).prop({ checked: false });
							topic_id = '0';
						} else {
							topic_id = $form[0].topic_id.value;
						}
					});
				}
			}
		...
		writeArticle: function () {
			...
			if ($form.is(':has(p.topics)')) {
				parameters.topic_id = ($form[0].topic_id || {}).value || 0;
			}
			$.ajax({
